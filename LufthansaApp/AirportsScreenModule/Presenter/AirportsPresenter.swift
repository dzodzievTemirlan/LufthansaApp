...
protocol AirportsViewProtocol: class {
  func success()//через этот метод передать данные
...
class AirportsPresenter: AirportsViewPresenterProtocol {
  ...
  func getAirportsFromLocal() {
    coreDataService?.fetchDataFromCoreData(key: .airports,
                                           to: .airports,
  ...
  func getAirportsFromNetwork() {
    if localStorageIsEmpty {
      self.networkConnection?.checkInternetConnection(complition: { (check) in
        if check {
          self.dataFetcher?.fetchJSONData(request: .airports,
              ...
              DispatchQueue.main.async {
                ...
                case .success(let airports):
                  ...
                  self.view?.success()
                  self.coreDataService?.saveDataFromNetwork(key: .airports,
                                                            to: .airports,
                                                            that: self.airports!)
