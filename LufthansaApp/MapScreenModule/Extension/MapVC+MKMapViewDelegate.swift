...
extension MapViewController: MKMapViewDelegate {
  func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
    ...
    if let view = mapView.dequeueReusableAnnotationView(withIdentifier:
                                                        String(describing:MKMarkerAnnotationView.self))
                                                        as? MKMarkerAnnotationView {
      view.annotation = annotation
      viewMarker = view
    } else {
      viewMarker = MKMarkerAnnotationView(annotation: annotation,
                                          reuseIdentifier:String(describing: MKMarkerAnnotationView.self))
