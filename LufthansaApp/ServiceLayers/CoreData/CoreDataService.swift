...
protocol CoreDataServiceProtocol {
    func saveDataFromNetwork<Elements>(
                                    key whichKey: KeyType,
                                    to entity: EntityNames,
                                    that model: [Elements] )
    func fetchDataFromCoreData<T: NSManagedObject>(
                                                key whichKey: KeyType,
                                                to entity: EntityNames,
                                                type: T.Type,
                                                compilition: @escaping ([T]?, Error?) -> Void)
}
...
class CoreDataService: CoreDataServiceProtocol {
    ...
    func saveDataFromNetwork<Elements>(
                                    ...
                                    that model: [Elements]) {
        guard let userEntity = NSEntityDescription.entity(
                                                        forEntityName: entityName.rawValue,
